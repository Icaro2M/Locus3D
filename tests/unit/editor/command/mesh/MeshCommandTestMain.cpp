/*
 * SPDX-FileCopyrightText: 2026 Icaro2M
 * SPDX-License-Identifier: Apache-2.0
 */

#include "MeshCommandTestSuite.h"

#include "common/TestLog.h"

#include <string>
#include <string_view>

namespace {

struct TestCase {
    std::string_view name;
    locus::tests::TestResult (*run)();
};

} // namespace

int main()
{
    using namespace locus::tests;

    const TestCase tests[] = {
        { "MeshSnapshot", run_mesh_snapshot_tests },
        { "ReplaceMeshCommand", run_replace_mesh_command_tests },
        { "ApplyMeshOperationCommand", run_apply_mesh_operation_command_tests },
        { "EditMeshSelectionCommand", run_edit_mesh_selection_command_tests },
    };

    for (const TestCase& test : tests) {
        const TestResult result = test.run();
        if (!result.success) {
            log_error(std::string(test.name) + ": " + result.message);
            return 1;
        }

        log_info(std::string(test.name) + ": passed");
    }

    return 0;
}
