...
- `PivotRenderer` receives graphics-only `PivotDrawData` and draws constant-pixel screen-space crosshair billboards with a central red point, four separated arms, and normal/hovered/active colors. It renders as an editor overlay with depth test and depth writes disabled by default, and owns no editor concepts such as `SceneNodeId`, `NodePivot`, selection, tools, commands, or history.
